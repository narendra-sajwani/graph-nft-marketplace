...
export class ListingCanceled extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ListingCanceled entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ListingCanceled must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ListingCanceled", id.toString(), this);
    }
  }

  static load(id: string): ListingCanceled | null {
    return changetype<ListingCanceled | null>(store.get("ListingCanceled", id));
  }
